        ...
        >
          {[
            ...
            <Link
              ...
              className={`nav-link ${hoveredNav === nav.id || (hoveredNav === null && nav.id === 'profile') ? 'nav-link-active' : 'nav-link-inactive'}`}
            >
              <nav.icon size={18} className="shrink-0" />
              <span className={`overflow-hidden text-xs font-semibold transition-all duration-300 ${hoveredNav === nav.id || (hoveredNav === null && nav.id === 'profile') ? nav.width + ' opacity-100 ml-1.5' : 'w-0 opacity-0'}`}>
                {nav.label}
              </span>
            </Link>
          ))}
